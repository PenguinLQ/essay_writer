...
import os
import sqlite3
from essay_writer.core.config import get_settings
...
class writer_gui( ):
    ...
    def run_agent(self, start, topic, stop_after):
        if start:
            ...
            config = {'task': topic, "max_revisions": 2, "revision_number": 0,
                      ...
                      'content': [], 'queries': "no queries", 'count':0}
    ...
    def modify_state(self,key,as_node,new_value):
        ''' gets the current state, modifies a single value in the state identified by key, and updates state with it.
        ...
        self.graph.update_state(self.thread, current_values.values, as_node=as_node)
        return
